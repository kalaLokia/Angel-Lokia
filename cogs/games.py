...
class Games(commands.Cog):
    ...
    @commands.command(name='bj', help='Blackjack Game')
    async def on_blackjack(self,ctx):
        ...
        response = await self.bot.wait_for('message', check=self.check(ctx.author), timeout=60)
        ...
        while(play):
            ...
            while(not bj.bust(bj.playershand)):
                ...
                await ctx.send(f"{bj.player.name}'s turn: Do you want to hit or stand ?")
                response = await self.bot.wait_for('message', check=self.check(ctx.author), timeout=60)
